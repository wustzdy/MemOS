...
def example_multi_db(db_name: str = "paper_polardb"):
    ...
    concepts = [
        ...
        TextualMemoryItem(
            ...
            metadata=TreeNodeTextualMemoryMetadata(
                ...
                updated_at=datetime.now().isoformat(),
                embedding=embed_memory_item(
                    "Coverage performance is measured using CT (Coverage Time) and FT (Fairness Time) metrics."
                ),
            ),
        ),
    ]
    ...
    for concept in concepts:
        ...
        print(f"Creating edge: ({concept.id}) -[:RELATED]-> ({topic.id})")
...
def run_user_session(
    ...
    """Run a complete user session with PolarDB."""
    ...
    graph.add_node('45678', topic.memory, topic.metadata.model_dump(exclude_none=True))
...
def example_complex_shared_db(db_name: str = "poc"):
    """Complex example with multiple users in shared database."""
